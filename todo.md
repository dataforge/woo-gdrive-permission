...
- ~~**`get_unassigned_order_items` decrements `$total` after pagination**~~ Partially fixed 2026-07-02 (v3.4.28): `class-wgdp-entitlements.php:1199-1206` now clamps with `max(0, $total)` so the Access Manager never shows a negative total. **Still open:** the total remains a page-local approximation — it can still vary slightly between pages because the COUNT query can't replicate the PHP-side qualification/refund checks. A full fix would move the qualification/qty filter into SQL (non-trivial, since qualification checks live in `WGDP_Product_Meta` and refund qty comes from `wc_get_order()`), or accept the approximation. Deferred.
