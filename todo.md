...
**Follow-up:** investigate a fix for the unindexed refund-lookup subquery added in v3.4.29 — `get_unassigned_order_items()` (`class-wgdp-entitlements.php:1140-1154`, `refund_totals` derived table) scans `wc_order_itemmeta`/`woocommerce_order_itemmeta` filtered by `meta_key = '_refunded_item_id'`. Re-validated 2026-07-02: confirmed neither the legacy nor HPOS itemmeta table indexes `meta_key`/`meta_value` (only `PRIMARY KEY(meta_id)` and `KEY order_item_id`), so this is a genuine full-table scan, run twice per page load (once in the `COUNT()` query, once in the paged query). No lightweight SQL rewrite avoids it — the join key is the meta *value*, not something a differently-shaped query can index around. Fine for typical stores; only worth chasing if the Access Manager page is reported slow. Two real directions if it is: (1) add a supplemental composite index via a `dbDelta`/upgrade routine on `(meta_key, meta_value(191))` — additive, no core WC table restructuring, turns the scan into an index lookup; (2) cache refund totals in a summary table updated on refund hooks — more invasive but avoids adding an index to a table this plugin doesn't own.
