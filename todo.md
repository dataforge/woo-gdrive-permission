...
**Follow-up:** investigate a fix for the unindexed refund-lookup subquery added in v3.4.29 — `get_unassigned_order_items()` (`class-wgdp-entitlements.php:1140-1154`, `refund_totals` derived table) scans `wc_order_itemmeta`/`woocommerce_order_itemmeta` filtered by `meta_key = '_refunded_item_id'`. Re-validated 2026-07-02 (session 3): confirmed still a genuine full-table scan (no index on `meta_key`/`meta_value`), run twice per page load. Also confirmed the plugin's own `dbDelta` upgrade routine (`class-wgdp-db.php`) only ever creates/manages its own two tables — it has no established pattern for altering a WooCommerce-owned table, and order items/itemmeta always resolve to the single core tables regardless of HPOS mode (no legacy/HPOS split to worry about for this specific query). Given no reported real-world slowness and the ongoing compat/maintenance risk of a third-party plugin indexing a WC-core table from its activation hook, recommendation is now firm: defer, and if it ever needs fixing prefer caching refund totals in a summary table this plugin owns (option b) over adding an index to `woocommerce_order_itemmeta` (option a).
