...
- **`WGDP_OTP::attempt_verification()` "too many attempts" `get_var()` re-read could misreport remaining attempts if the row is deleted mid-request** — `class-wgdp-otp.php:158-161` (the wrong-OTP branch's `$current_attempts = (int) $wpdb->get_var(...)`): if the entitlement row were deleted concurrently, `get_var()` returns `null`, `(int) null` coerces to `0`, and `$remaining` is computed as the full `MAX_OTP_ATTEMPTS` instead of reflecting reality. No current codepath deletes an entitlement row mid-verification, so this is a latent edge case, not a reachable bug today. Left open as a low-priority hardening item.
